...
p = pathlib.Path("lib/storeAccess.ts")
s = p.read_text()

old = '''import { NextRequest } from "next/server";
...
new = '''import { NextRequest } from "next/server";
import { db } from "@/lib/firebaseAdmin";
import { verifyOwnerToken } from "@/lib/ownerAuth";
...
if old not in s:
    raise SystemExit("storeAccess.ts content not found — aborting")
s = s.replace(old, new)
