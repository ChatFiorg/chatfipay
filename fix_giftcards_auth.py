...
p1 = pathlib.Path("app/api/store/[slug]/giftcards/route.ts")
s1 = p1.read_text()
...
if old_helper not in s1:
    raise SystemExit("giftcards GET block not found — aborting")
...
if old_post not in s1:
    raise SystemExit("giftcards POST block not found — aborting")
s1 = s1.replace(old_post, new_post)

p1.write_text(s1)
print("done giftcards list/create")

p2 = pathlib.Path("app/api/store/[slug]/giftcards/[code]/route.ts")
s2 = p2.read_text()
...
if old_helper2 not in s2:
    raise SystemExit("giftcards PATCH block not found — aborting")
...
if old_delete not in s2:
    raise SystemExit("giftcards DELETE block not found — aborting")
s2 = s2.replace(old_delete, new_delete)

p2.write_text(s2)
print("done giftcards detail")
